...
public class CurrencyBot extends TelegramLongPollingBot {
    ...
    public void onUpdateReceived(Update update) {
        ...
        switch (state) {
            case CONVERSATION_STARTED -> {
                message.setText("Ëàñêàâî ïðîñèìî!\n" +
                        "Öåé áîò äîïîìîæå â³äñë³äêîâóâàòè àêòóàëüí³ òà àðõ³âí³ êóðñè âàëþò");
                message.setReplyMarkup(setupBeginButton());
                userContext.get(chatId).setState(WAITING_FOR_CHOISE);
            }
            case WAITING_FOR_CHOISE -> {
                if (update.getMessage().getText().equalsIgnoreCase("Íàëàøòóâàííÿ")) {
                    System.out.println(update.getMessage().getText());
                    message.setText("Çì³íà íàëàøòóâàíü");
            ...
            case WAITING_FOR_SETTING -> {
                if (update.getMessage().getText().equalsIgnoreCase("ÍÀÇÀÄ")) {
                    System.out.println(update.getMessage().getText());
                    message.setText("Âèáåð³òü \u261f");
                    message.setReplyMarkup(setupBeginButton());
                    userContext.get(chatId).setState(WAITING_FOR_CHOISE);
                } else if (update.getMessage().getText().equalsIgnoreCase("ÂÀËÞÒÀ")) {
                    message.setText("Âèáåð³òü âàëþòó");
                    message.setReplyMarkup(setupCurrencyKeyboard(chatId));
                    userContext.get(chatId).setState(GETTING_CURRENCY);
                } else if (update.getMessage().getText().equalsIgnoreCase("ÁÀÍÊ")) {
                    message.setText("Âèáåð³òü áàíê. Ìîæíà äåê³ëüêà");
                    message.setReplyMarkup(setupBankKeyboard(chatId));
                    userContext.get(chatId).setState(GETTING_BANK);
                } else if (update.getMessage().getText().equalsIgnoreCase("ÔÎÐÌÀÒ")) {
                    message.setText("Âèáåð³òü ðàçðÿäí³ñòü çíà÷åííÿ êóðñó");
                    message.setReplyMarkup(setupBitDepthKeyboard(chatId));
                    userContext.get(chatId).setState(GETTING_FORMAT);
                } else if (update.getMessage().getText().equalsIgnoreCase("ÑÏÎÂ²ÙÅÍÍß")) {
                    message.setText("Êîëè Âè õî÷åòå áà÷èòè ãàðíèé êóðñ?");
            ...
            case GETTING_CURRENCY -> {
                ...
                message.setText("Îáðàíî: " + userContext.get(chatId).getCurrency());
                message.setReplyMarkup(setupSettingKeyboard());
                userContext.get(chatId).setState(WAITING_FOR_SETTING);
            }
            case GETTING_BANK -> {
                if (update.getMessage().getText().equalsIgnoreCase("ÍÀÇÀÄ")) {
                    ...
                    message.setText("Îáðàíî: " + ((count > 1) ? count + " áàíêè" : count + " áàíê"));
                    message.setReplyMarkup(setupSettingKeyboard());
                    userContext.get(chatId).setState(WAITING_FOR_SETTING);
                } else {
                    ...
                    if (userContext.get(chatId).getBank().isEmpty()) {
                        ...
                        message.setText("Çà çàìîâ÷óâàííÿì âèáðàíî: " + PRIVAT);
                    }
                    message.setText("Ok");
                    message.setReplyMarkup(setupBankKeyboard(chatId));
                    userContext.get(chatId).setState(GETTING_BANK);
            ...
            case GETTING_FORMAT -> {
                if(Integer.parseInt(update.getMessage().getText().split(" ")[0]) < 5) {
                    ...
                    message.setText("Îáðàíî: " + userContext.get(chatId).getBitDepth() + " çíàêè ï³ñëÿ êîìè");
                } else {
                    userContext.get(chatId).setBitDepth(4);
                    message.setText("Öå âæå çàíàäòî. Âèñòà÷èòü ³ 4-îõ çíàêè ï³ñëÿ êîìè");
                }
                message.setReplyMarkup(setupSettingKeyboard());
                userContext.get(chatId).setState(WAITING_FOR_SETTING);
            ...
            case GETTING_REMINDER -> {
                ...
                message.setReplyMarkup(setupTimeReminderKeyboard(userContext.get(chatId).getHour(), userContext.get(chatId).getMinute()));
                if(update.getMessage().getText().equals("Âñòàíîâèòè ÷âñ")) {
                    message.setText("×àñ ñïîâ³ùåííÿ êîæíîãî äíÿ î " + userContext.get(chatId).getHour() + ":" + userContext.get(chatId).getMinute());
                    message.setReplyMarkup(setupSettingKeyboard());
                    userContext.get(chatId).setState(WAITING_FOR_SETTING);
                } //else return;
            ...
            default -> {
                message.setText("Íå òðåáà í³÷îãî ââîäèòè. Ò³ëüêè òèöÿéòå êíîïêè");
    ...
    private ReplyKeyboardMarkup setupTimeReminderKeyboard(String hour, String minute) {
        ...
        row1.add("Âñòàíîâèòè ÷âñ");
        row2.add("\u25b2"); row2.add("\u25b3");
        row3.add(hour + " ãîä"); row3.add(minute + " õâ");
        row4.add("\u25bc"); row4.add("\u25bd");
        row5.add("Â³äêëþ÷èòè ñïîâ³ùåííÿ");
